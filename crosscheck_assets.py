...
def crosscheck_assets(table_name): 
    """
    ...
    try:
        ...
        for file in flist: 
            ...
            if xml_status != None: 
                if xml_status[1] != 1:
                    index = xml_status[0]
                    db.update_row(table_name, index, 'xml_created', 1)
                    xml_status_msg = f" DB updated on crosscheck - \n\
                                        rowid: {index}, \n\
                                        guid: {guid} \n\
                                        xml_created: 1 \n"
        ...
        for file in plist:
            ...
            if proxy_status != None: 
                if proxy_status[1] != 1:
                    ...
                    db.update_row(table_name, index, 'proxy_copied', 1)
                    proxy_status_msg = f" DB updated on crosscheck - \n\
                                        rowid: {index}, \n\
                                        guid: {guid} \n\
                                        proxy_copied: 1 \n"
